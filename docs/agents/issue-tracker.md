...
- **Relay**: when you pass a decision to another session, name its source, strength and time — `Skylar ruled X at 21:05, asked directly` and `session <id> recommended X at 20:50` are different claims, and only the first settles anything. Carry the evidence and its strength rather than a verdict: a casual yes to a yes/no question is not a ruling, and a claim that has passed through two sessions is weaker than either hand that carried it. Read the clock rather than estimating elapsed time — an invented timestamp discredits the measured content beside it.
